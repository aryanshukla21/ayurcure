...
import { ShieldCheck, CalendarCheck, ChevronLeft, Loader2, AlertCircle } from 'lucide-react';
...
const ConsultationPaymentPage = () => {
    ...
    // BYPASSED PAYMENT LOGIC
    const handleConfirmBooking = async () => {
        setIsProcessing(true);
        const { bookingData } = paymentData;

        try {
            // Instantly Save to Database bypassing Razorpay
            const payload = {
                doctorId: bookingData.doctorId,
                date: bookingData.date,
                time: bookingData.time,
                reason: bookingData.reason,
                paymentId: 'PAYMENT_BYPASSED_TEMP' // Dummy ID for backend database
            };
        ...
        <div className="max-w-[1200px] mx-auto p-10 bg-[#FDF9EE] min-h-full">
            ...
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    ...
                    <p className="text-gray-500 font-medium mt-1">Review your details to finalize the appointment</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                <div className="bg-white rounded-[32px] p-8 border border-[#EFEBE1] shadow-sm relative overflow-hidden">
                    <div className="absolute top-0 right-0 bg-green-50 text-[#4A7C59] px-4 py-2 rounded-bl-2xl font-bold text-xs flex items-center gap-1 border-b border-l border-green-100">
                        <ShieldCheck size={14} /> Encrypted Booking
                    </div>

                    <h3 className="text-xl font-bold mb-6 mt-4">Booking Details</h3>

                    <div className="bg-gray-50 rounded-2xl p-6 mb-8 border border-gray-100">
                        <p className="text-sm text-gray-500 font-bold uppercase tracking-wider mb-2">Total Estimated Cost</p>
                        <p className="text-4xl font-extrabold text-gray-900">₹{financials.total}</p>
                        <p className="text-xs text-amber-600 font-bold mt-2 bg-amber-50 inline-block px-2 py-1 rounded">Payment skipped for now</p>
                    </div>
                    ...
                    >
                        {isProcessing ? <><Loader2 className="animate-spin" size={24} /> Processing...</> : 'Confirm Appointment'}
                    </button>
            ...
            {/* THE FIX: Passed the correct functions to the Modal so the buttons work! */}
            <AppointmentSuccessModal
                isOpen={paymentStatus === 'success'}
                onViewAppointment={() => navigate('/patient/appointments', { replace: true })}
                onGoToDashboard={() => navigate('/patient/dashboard', { replace: true })}
                appointmentDetails={{
                    doctorName: bookingData.doctorName,
                    date: bookingData.date,
                    time: bookingData.time,
                    id: 'PAY-BYPASSED-SUCCESS'
                }}
            />
            ...
            {paymentStatus === 'cancelled' && (
                <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-[32px] p-10 max-w-md w-full shadow-2xl text-center relative animate-in fade-in zoom-in duration-300">
                        ...
                        <h2 className="text-3xl font-extrabold mb-4 text-gray-900">Booking Cancelled</h2>
                        <p className="text-gray-500 font-medium mb-8">Your appointment has not been booked. You can modify your details and try again.</p>
                        <button
                            onClick={() => navigate('/patient/book-appointment', { replace: true })}
