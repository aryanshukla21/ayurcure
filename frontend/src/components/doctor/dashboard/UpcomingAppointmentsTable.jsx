                    ...
                    <tbody className="divide-y divide-transparent">
                        ...
                        {appointments.map((apt, index) => (
                            <tr
                                ...
                                onClick={() => navigate(`/doctor/appointments/${apt.id || apt._id}`)}
                            ...
                            >
                                <td className="px-3 py-2 rounded-l-2xl">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <span className="font-bold text-gray-900 text-xs">{apt.name || apt.patient_name}</span>
                                    </div>
                                </td>
                                <td className="px-8 py-5">
                                    <span className="text-gray-600 font-semibold text-xs">{apt.time || new Date(apt.start_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                </td>
                                <td className="px-8 py-5">
                                    <div className="flex items-center gap-3 text-gray-600">
                                        {apt.type === 'Video' || apt.mode === 'Video' ? <Video size={20} className="text-blue-500" /> : <User size={20} className="text-green-500" />}
                                        <span className="font-semibold text-xs">{apt.type || apt.mode}</span>
                                    </div>
                                </td>
                                <td className="px-8 py-5 rounded-r-2xl">
                                    <span className={`px-4 py-2 rounded-full text-xs font-bold tracking-wide ${getStatusStyle(apt.status)}`}>
                                        {apt.status}
                                    </span>
                                </td>
                            </tr>
