...
const PatientsTable = () => {
  ...
  const currentItems = patientsData.slice(startIndex, startIndex + ITEMS_PER_PAGE);
    ...
    <div className="bg-white rounded-[32px] p-8 border border-[#EFEBE1] shadow-sm flex flex-col h-full">
          ...
          <div key={patient.id} className="flex items-center py-4 border-b border-transparent hover:border-[#EFEBE1] hover:bg-[#FDF9EE]/50 rounded-2xl transition-colors group px-2 -mx-2">
            ...
            <div className="w-[10%]">
              <span className={`px-3 py-1.5 rounded-full text-[9px] font-extrabold uppercase tracking-widest ${
                patient.status === 'ACTIVE' ? 'bg-[#E7F3EB] text-[#3A6447]' : 'bg-[#FDF1E8] text-[#D9774B]'
              }`}>
                {patient.status}
              </span>
            </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mt-6 pt-6 border-t border-[#EFEBE1] gap-4">
        <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">
          Showing {startIndex + 1}-{Math.min(startIndex + ITEMS_PER_PAGE, patientsData.length)} of {patientsData.length} patients
        </p>
