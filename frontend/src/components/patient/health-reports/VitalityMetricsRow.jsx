    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="md:col-span-2 bg-[#EFECE5] rounded-[24px] p-6 flex flex-col justify-between">
        <div>
          ...
          <h3 className="text-xl font-extrabold text-gray-900">Overall Health Score: 92/100</h3>
        </div>

        {/* Mock Bar Chart */}
        <div className="flex items-end gap-2 h-16 mt-4">
          {['h-6', 'h-8', 'h-10', 'h-10', 'h-12', 'h-14', 'h-16'].map((height, i) => (
            <div key={i} className={`flex-1 rounded-t-md ${i === 6 ? 'bg-[#3A6447]' : 'bg-[#D1CFC8]'}`} style={{ height: i === 6 ? '100%' : `${(i + 3) * 10}%` }}></div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-[#F4EBD9] rounded-[24px] p-6 flex flex-col justify-center">
          ...
          <h3 className="text-3xl font-extrabold text-gray-900">3/4</h3>
        </div>
        ...
        <div className="bg-[#EFECE5] rounded-[24px] p-6 flex flex-col justify-center">
          ...
          <h3 className="text-sm font-extrabold text-gray-900 leading-tight">Yesterday,<br />04:30 PM</h3>
        </div>
      </div>
