            ...
            <div key={apt.id || apt._id} className="grid grid-cols-12 gap-4 px-8 py-6 items-center hover:bg-[#FAFAF8] transition-colors">
              ...
              <div className="col-span-4 flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-[#EAE5D9] flex items-center justify-center text-[#4A7C59] font-bold border border-[#EFEBE1] shrink-0">
                  {apt.doctorName ? apt.doctorName.charAt(4) : 'D'}
                </div>
                <div>
                  <p className="text-base font-bold text-gray-900">{apt.doctorName}</p>
                  <p className="text-sm text-gray-500 font-medium">{apt.specialty || apt.specialization}</p>
                </div>
              </div>

              <div className="col-span-2 text-sm font-semibold text-gray-700">
                {apt.date || new Date(apt.scheduled_at).toLocaleDateString()}
              </div>
              <div className="col-span-2 text-sm font-semibold text-gray-700">
                {apt.time || new Date(apt.scheduled_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
              </div>
              ...
              <div className="col-span-2 text-right">
                {apt.status?.toLowerCase() === 'cancelled' ? (
                  <button
                    ...
                    className="px-6 py-2 text-sm font-bold text-gray-700 bg-white border border-gray-300 rounded-full hover:bg-gray-50 transition-colors shadow-sm"
                  >
                    Rebook
                  </button>
                ) : (
                  <button
                    onClick={() => navigate(`/patient/appointments/${apt.id || apt._id}`)}
                    className="px-6 py-2 text-sm font-bold text-white bg-[#4A7C59] rounded-full hover:bg-[#3d6649] transition-colors shadow-sm"
                  >
                    {apt.status?.toLowerCase() === 'completed' ? 'View Summary' : 'View Appointment'}
                  </button>
                )}
              </div>
            </div>
          ...
          <div className="flex gap-2">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <button
                ...
                className={`w-9 h-9 flex items-center justify-center rounded-full text-sm font-bold transition-colors shadow-sm ${currentPage === page
